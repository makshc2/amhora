...
async function signTransaction(txHex, seed, addresses) {
  ...
  txHex.inputs.forEach((item, index) => {
    ...
    let address = bitcoin.address.fromOutputScript(prevTx.outs[input.index].script, DASH_TESTNET_NETWORK)
  ...
  tx.outs.forEach(item => {
    let address = bitcoin.address.fromOutputScript(item.script, DASH_TESTNET_NETWORK)
  ...
  derivationIndexes.forEach(item => {
    const DERIVE = root.derive(`m/44'/1'/1'/0/${item}`),
      KEY_PAIR = bitcoin.ECPair.fromPrivateKey(DERIVE.privateKey, DASH_TESTNET_NETWORK),
      WIF = KEY_PAIR.toWIF()
...
function generatePrivateKey(seed, derivationNum) {
  ...
  const keyPair = bitcoin.ECPair.fromPrivateKey(addrnode.privateKey, { network: DACH_TESTNET_NETWORK })
